  ...
  if (!listing) {
    return (
      <div className="max-w-6xl mx-auto px-6 py-20 text-center">
        ...
        <Link href="/browse" className="text-accent text-sm mt-4 block">Back to browse</Link>
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto px-6 py-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div className="space-y-4">
          ...
          <div className="flex items-start gap-2 p-3 rounded-xl bg-accent/5 border border-accent/20">
            ...
            <p className="text-xs text-ink-400 leading-relaxed">
              ...
              <span className="text-ink-200">The creator&apos;s review is your proof of condition.</span>{" "}
        ...
        <div>
          ...
          <h1 className="text-2xl font-medium text-ink-50 mb-2 leading-snug">{listing.title}</h1>

          <div className="flex items-baseline gap-3 mb-6">
            <span className="text-3xl font-medium text-ink-50">{formatPrice(listing.price)}</span>
            {listing.original_price && (
              <span className="text-ink-500 line-through text-base">{formatPrice(listing.original_price)}</span>
            )}
            {discount && (
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  {creator.youtube_subscriber_count && (
                    <p className="text-xs text-ink-500">{formatSubscriberCount(creator.youtube_subscriber_count)} subscribers</p>
                  )}
                </div>
              </div>
          ...
          <p className="text-center text-xs text-ink-600 mt-3">
            Secure checkout via Razorpay · Ships via Shiprocket
          </p>
        </div>
      </div>
    </div>
